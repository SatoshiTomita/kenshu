...
def run_replay(cfg, model: nn.Module, state_norm: Normalizer, action_norm: Normalizer, device: torch.device, fig_dir: Path):
    ...
    action_count: np.ndarray | None = None
    ...
    for step_idx in range(int(cfg.replay.steps)):
        ...
        with torch.no_grad():
            if input_len == 1:
                ...
                action_seq, h = model(image_in, state_in, h=None)
            ...
            if action_seq.ndim == 4:
                ...
                chunk_actions_np = future
            else:
                chunk_actions_np = action_seq[0, -1].detach().cpu().numpy()[None, :]
